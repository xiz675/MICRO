...
def _pad_and_combine(x):
    ...
    # this is the feature dimension for each node, so it should be the same
    second_dim = x[0].shape[1]
    for itm in x[1:]:
        assert itm.shape[1] == second_dim
    # the maximum number of nodes among the trees
...
def prepare_trees(trees, transformer, left_child, right_child, cuda=False, device=None):
    flat_trees, indexes = [], []
    for x in trees:
        if x.neo4j_tensor is not None:
            tree_tensor, index_tensor = x.neo4j_tensor
        else:
            flat_tree = _flatten(x, transformer, left_child, right_child)
            index = _tree_conv_indexes(x, left_child, right_child)

            x.neo4j_tensor = (flat_tree, index)
            tree_tensor, index_tensor = flat_tree, index
        flat_trees.append(tree_tensor)
        indexes.append(index_tensor)

    flat_trees = _pad_and_combine(flat_trees)
    flat_trees = torch.tensor(flat_trees, dtype=torch.float32, device=device).transpose(1, 2)

    indexes = _pad_and_combine(indexes)
    indexes = torch.tensor(indexes, dtype=torch.long, device=device)

#    flat_trees = torch.cat(flat_trees, dim=0)
#    indexes = torch.cat(indexes, dim=0)
    #
    # flat_trees = [_flatten(x, transformer, left_child, right_child) for x in trees]
    # flat_trees = _pad_and_combine(flat_trees)
    # flat_trees = torch.tensor(flat_trees, dtype=torch.float32)
    #
