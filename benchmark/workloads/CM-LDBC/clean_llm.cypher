...
MATCH (f:Forum)-[:HAS_MEMBER]->(p:Person)-[:HAS_CREATOR]->(msg:Message)-[:REPLY_OF]->(orig:Message) WHERE f.title = $param AND orig.creationDate > $param RETURN p.firstName, p.lastName, msg.content;
...
MATCH (person:Person)-[:LIKES]->(msg:Message)-[:IS_LOCATED_IN]->(c:City)<-[:IS_LOCATED_IN]-(uni:University) WHERE c.name = $param AND uni.name = $param RETURN person.firstName, person.lastName, msg.content;
...
MATCH (p:Person)-[:STUDY_AT]->(uni:University)-[:IS_LOCATED_IN]->(country:Country) WHERE uni.name = $param RETURN p.firstName, p.lastName, country.name;
...
MATCH (p:Person)-[:HAS_CREATOR]->(msg:Message)-[:REPLY_OF*]->(origMsg:Message)<-[:HAS_CREATOR]-(origAuthor:Person) WHERE origAuthor.birthday < $param RETURN p.firstName, p.lastName, msg.content, origAuthor.firstName, origAuthor.lastName;
MATCH (p:Person)-[:KNOWS]->(friend:Person)-[:WORK_AT]->(comp:Company)-[:IS_LOCATED_IN]->(city:City) WHERE city.name = $param RETURN p.firstName, p.lastName, friend.firstName, friend.lastName, comp.name, city.name;
MATCH (p:Person)-[:KNOWS]-(friend:Person)-[:HAS_CREATOR]-(m:Message)-[:HAS_TAG]->(t:Tag)-[:HAS_TYPE]->(tc:TagClass)
WHERE p.firstName = $param AND tc.name = $param
...
MATCH (post:Post)-[:IS_LOCATED_IN]->(place:Place)<-[:IS_PART_OF]-(country:Country)<-[:IS_PART_OF]-(continent:Continent) 
...
MATCH (p:Person)-[:HAS_INTEREST]->(t:Tag)<-[:HAS_TAG]-(post:Post)-[:CONTAINER_OF]->(forum:Forum)
   ...
   RETURN forum.title, post.content;
MATCH (person:Person)-[:STUDY_AT]->(uni:University)-[:IS_LOCATED_IN]->(city:City)<-[:IS_LOCATED_IN]-(company:Company)<-[:WORK_AT]-(colleague:Person)
   WHERE person.email = $param AND colleague.birthday < $param
   RETURN colleague.firstName, colleague.lastName, company.name;
MATCH (msg:Message)-[:REPLY_OF]->(orig:Message)<-[:HAS_CREATOR]-(creator:Person)-[:WORK_AT]->(comp:Company)
...
MATCH (m1:Message)-[:REPLY_OF]->(m2:Message)-[:HAS_CREATOR]->(p:Person)-[:KNOWS]->(p2:Person) WHERE m1.creationDate > $param AND p2.locationIP = $param RETURN m1.content, m2.content, p.firstName, p2.firstName;
...
MATCH (p:Person)-[:WORK_AT]->(c:Company)-[:IS_LOCATED_IN]->(city:City)-[:IS_PART_OF]->(country:Country) WHERE p.gender = $param AND city.name = $param RETURN p.firstName, p.lastName, c.name, country.name;
...
MATCH (company:Company)<-[:WORK_AT]-(p:Person)-[:KNOWS]->(friend:Person)-[:HAS_CREATOR]->(m:Message)-[:IS_LOCATED_IN]->(city:City) WHERE company.name = $param AND city.name = $param RETURN friend.id, m.content;
...
MATCH (p:Person)-[:WORK_AT]->(c:Company)-[:IS_LOCATED_IN]->(pl:Place)
   ...
   RETURN p.firstName, p.lastName, c.name, pl.name;
MATCH (p:Person)-[:HAS_CREATOR]->(m:Message)-[:REPLY_OF]->(o:Message)-[:HAS_CREATOR]->(op:Person)
