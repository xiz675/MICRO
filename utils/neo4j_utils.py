...
class Neo4jUtil:
    ...
    def run_query_stream(self, query, pg_util: PostgresUtil, batch_size=4*1024*1024, max_records=1000000):
    ...
    def generate_filter_table(self, query: str, table_name: str, pg_util: PostgresUtil, batch_size=4*1024*1024, max_records=1000000):
        """Stream results from Neo4j and process them in batches."""
        try:
            with self._driver.session() as session:
                start_time = time.time()
                ...
                dtypes = [int.__name__]
                pg_util.run_query(pg_util.create_pg_table(table_name, ['id'], dtypes))
                # prepare copy sql
                copy_sql = f"COPY {table_name} (id) FROM STDIN WITH CSV"
